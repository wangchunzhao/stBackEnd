...
public interface CustomerRepository extends JpaRepository<DCustomer, Integer>,JpaSpecificationExecutor<DCustomer>{
	...
	@Query(value = "SELECT * FROM bohemian.sap_customer where  name like %:name%",
		    countQuery = "SELECT count(*)  FROM bohemian.sap_customer where  name like %:name%",nativeQuery=true)
