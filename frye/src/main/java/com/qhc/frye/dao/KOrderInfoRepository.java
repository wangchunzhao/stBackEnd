...
public interface KOrderInfoRepository extends JpaRepository<KOrderInfo, Integer>, JpaSpecificationExecutor<KOrderInfo> {
	...
	@Query(value = "SELECT * FROM k_orders left join k_order_version on k_orders.id = k_order_version.k_orders_id left join k_order_info on k_order_version.k_order_info_id = k_order_info.id where k_orders.sequence_number = :seqNum and k_order_version.version=:version", nativeQuery = true)
