...
import com.qhc.order.dao.ApplicationRepository;
...
public class UserRoleService {
	...
	public List<UserRole> findByBUsersId(int userId) {
		return applicationRepository.findByBusersId(userId);
	}
	
	public UserRole add(UserRole applicationOfRolechange) {
	...
	public void addOrUpdateApp(User user) throws Exception{
		...
		//得到application信息
		UserRole oldApp = null;
		String roles = "";
		List<UserRole> apps = findByBUsersId(user.getId());
		if(apps!=null&&apps.size()>0) {
			oldApp = apps.get(0);
			for(UserRole app : apps) {
			...
			if(roleList!=null&&roleList.size()>0) {
				String roleIds = "";
				//修改人保存的位置，得到修改人
				UserRole creatorContainer = user.getApps().get(0);
